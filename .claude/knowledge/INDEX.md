...
**NPC / dialog:** `talk_trigger.md` (THE blocker — `+0x200` bit 0x400), `dialog_native_roadmap.md` (Path A, current), `dialog_runtime.md`, `dlgnpc_bind.md`, `npc_model.md` (⚠️ flag rows superseded), **`npc_ai_flags.md`** (2026-09-10 — `+0x1F4` = AI behaviour-mode word with the full mode table, `+0x200` = service/role bits incl. smith/merchant/trainer, stance→faction class, the `Ki*`/`Ei*` state-machine map; static + 64k-record corpus evidence), `npc_templates.md`, `combat_init.md` (HP/AI init, hostility matrix @0x890A30), `runtime_spawn.md`, `triggers_dialog_move.md` (move/teleport/voice command ring).
**Binary sweep / tooling:** **`text_sweep_2026-09.md`** (Ghidra coverage 78.8 %→93.2 % and how to redo it, the German-log AI naming trick, the built-in developer console `FUN_00615550` gated by `DAT_0182ee38 & 0x2000000`, and the native balance-key parser `FUN_005eb010` → 227 named live globals at `0x00AD4490..0x00AD55A8`, dumped in `fixtures/balance_keys.txt`). Tooling lives in `sdk/re/py/x86/`.
**Items / save:** `items.md`, `items_equip.md` (equip ABI, TYPE space @0x008EC328), `gold_safe.md` (+0x3EE save-corruption fix), `class_mod_feasibility.md` (2026-09-16: class slot replacement — research, NPC body candidates, and the live-tested `lib/classmod.lua` implementation; evidence in `re/classes/`).
