# Disasm contexts where ecx=0xAB44AC container methods are invoked
import struct, capstone

EXE = r"E:\SteamLibrary\steamapps\common\Sacred Gold\sdk\Sacred_decrypted.exe"
IMGBASE = 0x00400000
SITES = [0x0052C4C0, 0x0052C4F4, 0x0052C5C8, 0x0055DD0B, 0x00593F42, 0x00597F41, 0x007E7ADE]
...
def cstr(va, maxlen=120):
    ...
    try:
        t = s.decode('latin1')
        if t and len(t) >= 2 and all(32 <= ord(c) < 127 for c in t):
            return t
    except Exception:
        pass
    return None
...
for site in SITES:
    print("=== context @ 0x%08X ===" % site)
    ctx = site - 0x30
    o = va2off(ctx)
    for ins in md.disasm(d[o:o + 0x70], ctx):
        note = ''
        ...
        print("0x%08X  %-8s %s%s%s" % (ins.address, ins.mnemonic, ins.op_str, note, mark))
    print()
