# Full disasm of our FIX-A2 binder FUN_00463240 — what does it do with the
# Talk_%s_Dlg_%d key it formats at 0x4632E8? Does it touch 0xAB44AC?
...
o = va2off(0x463240)
for ins in md.disasm(d[o:o + 0x500], 0x463240):
    note = ''
    for tok in ins.op_str.replace('[', ' ').replace(']', ' ').replace(',', ' ').split():
        if tok.startswith('0x'):
            ...
            if 0x00870000 <= v <= 0x009F0000:
                s = cstr(v)
                if s: note = '   ; "%s"' % s
            if v == 0xAB44AC: note = '   ; <<< named-trigger container 0xAB44AC'
