...
The 2.29 patch is the foundation of ReBorn HD — ReBorn is these 7 fixes from
2007 plus a decade of additional changes. Having the source means the changes
are known exactly rather than recovered by bindiff.
...
keeping the SEH frame. PE BINARY resource 107 can then be deleted — which is
why ReBorn HD's `.rsrc` is 2.7 MB smaller than vanilla.

**For us (DLL hook)**: hook `FUN_0080e680` at `va:0x0080E680`. When called,
return the disk file's bytes instead of the resource. Return contract:
`puVar4 = operator_new(size); *in_ECX = puVar4; in_ECX[1] = size;`. Simpler
alternative: hook `FindResourceA` with `lpName=0x6B, lpType="BINARY"` and return
a fake HRSRC backed by a disk read.

This hook unlocks:
- Runtime language switching (read `scripts/<lang>/global.res` from any path;
  pick `lang` in the ImGui overlay).
...
**For us**: irrelevant. As a DLL we have all the memory we want. Skip.
...
**For us**: low priority (single-player doesn't trigger it). For MP: MinHook on
the chat-handler function with the same IsBadReadPtr wrap.

### Change 5 — Version display "2.29.12.0"
Cosmetic — main menu shows new version, but internal `VL_VERSION` stays 2.28 so
multiplayer lobby still accepts the client.

**For us**: hook the UI render function or patch the version string at
`0x0095D014` directly. Lowest priority.
...
This is the same bug that triggered when force-borderless was attempted: hooking
`ChangeDisplaySettingsA` to swallow the call left Sacred's force-focus afterward
in an undefined state, producing the black screen + frozen taskbar.
...
**For us**: irrelevant unless running multiple Sacred copies from one install.
Skip.

## Our build vs theirs

`tools/verify_229_patches.py`: the byte patterns at the exact 2007 patch
addresses do not match, because Steam Gold 2.28 ASE (2006-10-13) has a slightly
different binary layout from the German 2.28 the patch targeted. However:

- `FUN_0080e680` matches byte-for-byte at `va:0x0080E680` — the PE BINARY
  loader is the same.
- Function `0x00856A95` (Balance.bin path build) is 11 bytes shifted vs patch's
  `0x00856A8A` — same function, slightly different prologue.
- The string compare anchor at `0x0094C4D4` has different content (us:
  `"VL_TIME_TAG"`, them: probably `".\\Bin\\Balance.bin"`).

All 7 patches are conceptually transferable; the sites need re-discovery in our
build:
...
1. Hook `FUN_0080e680` (Change 1) — runtime `global.res` from disk; unlocks
   language switching + text mods.
2. Relocate + NOP focus-force (Change 6) — removes the black-screen risk, makes
   future DDraw / display hooks safe to attempt.
...
- Change 3 (no content).
- Change 7 (single-instance is fine).
...
Originals archived in `sdk/docs/refs/`.
