...
This document is the human synthesis on top of the auto-generated tag table in [04-funkcode-tags.md](04-funkcode-tags.md). It pins down the framing, the payload grammar, the type-tag dictionary, the symbol inventory, and the deterministic sub-record chains.

All evidence comes from `bin\TYPE_NPC_SERAPHIM\FunkCode.bin` (125 055 records, 3 962 707 bytes); cross-checks with the other 7 base classes are noted where they applied.
...
The whole 4 MB file tiles cleanly into **125 055 records** with zero leftover. There is no nesting at the framing level — the stream is flat.

Note the size width: 16-bit **big-endian**. The first 5388 records the file contains are all under 256 bytes, so the high byte (`payload offset 0` minus 3 bytes earlier) is always `0x00`, which made us briefly think it was a 3-byte size; it isn't.
...
Walking the stream with this rule **fully consumes 117 681 of 117 681 non-marker payloads (100%)**. Every byte is accounted for. There are no length-prefixed blocks at this level — the next `type:u8` determines exactly how many bytes the value occupies.
...
| `0x1c` | `u32 LE` | 9 350 | integer, distinct role (frequently used as a key/index in chain records). |
| `0x38` | `u32 LE` | 2 038 | integer, smaller values; e.g. `0x000005a0` (=1440) in `z` records. |
| `0x16` | `cstr` (ASCIIZ) | 1 871 | string **literal** (vs. `0x01` which is a name reference). |
...
Types still **opaque** (their byte width isn't a clean u32 and the role is unclear): `0x02` (33 786, mostly 35/36/31-byte payloads — likely length-prefixed strings or arrays), `0x36` (5536, large variable payload), `0x04`, `0x1e`, `0x7d`, `0x48`, `0x2a`, `0x28` (always 2 bytes), `0x7c`, `0x52`, `0x75`, `0x09`, `0x8f`, `0x31`, etc.

Resolving them is the natural next step (probe specific record/type combinations).
...
17 tags share this shape: `[tag:1][size=0x0009][flags:1=00][type:1][value:4]`.
...
These are essentially **single-operand opcodes / one-field nodes**.
...
`0x73 's'` (22 762 — most frequent statement-like), `0x64 'd'` (data), `0x42 'B'` (block), `0x43 'C'` (constant decl), `0x69 'i'` (init), `0x3a ':'` (label), `0x3b ';'` (end), `0x3c '<'` (open), `0x68 'h'` (hook), `0x7d '}'` (sentinel-block), `0x1f` (template-name), `0x1a` (class-tag), …

Approximately 60 of these. Their inner field-streams obey §2 and can be decoded with the type-tag dictionary.
...
Every transition is 100 % deterministic, except `21 ! → 76 v` which holds 1848/1870 (99 %), and the chain's exit `1f → (01 or 7a)`.
...
`n` ranges 1..23 (DQ14 absent — suggests a numbering hole). Multiplicities (22, 23) likely correspond to **per-region replication** since Sacred has ~23 region zones.
...
2. **Cross-reference template names with `scripts\us\global.res`** — `0x16` literals like `SOUND_FX_VERRUECKTER_SFX` almost certainly resolve to localized resources.
3. **Locate the parser in `Sacred.exe`** by xref'ing the `DQ_NUM` / `GNr` strings (they'll appear in `.data`). That gives us field-type interpretations confirmed by the compiler/parser.
4. **Symmetric encoder** — once §1–§3 are solid, writing a *FunkCode → text → FunkCode* round-trip is a weekend's work.
...
- [`sdk\tools\funkcode_typemap.py`](../tools/funkcode_typemap.py) — proves the §2 payload grammar by attempting to fully parse every record's field stream; dumps type-tag histogram, daily-quest templates, `d`-record field distributions, and named-symbol inventory.
