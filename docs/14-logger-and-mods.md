...
Three things landed in one pass:
...
3. **HD-resolution force** (`sdk/hooks.cpp` + `sdk.ini`) — borderless windowed
   at custom resolution, gated by config, finally safe to use because
   `patch6` killed the busy-wait that bricked us last time.
...
`sdk/logs/seen_hashes.csv` grows continuously (flush every 5 s). Each line
is one hash. Use `tools/seen_hashes_resolve.py` to join against
...
Run this after a play session to see what was actually queried.

### Why it's high-value

- Live coverage of the **whole** id space the game uses (not just what we
  manually templated).
- "Unnamed hashes" in the output are perfect reverse-engineering targets —
  we know they're real, with real text, just unknown symbolic name.
- Combined with future-us writing a unicorn-emulator-driven brute-forcer,
  the unnamed set becomes much smaller.

## 2. Text-mod workflow

Patch 1 makes Sacred read `global.res` from disk on every load. So a text
mod is literally "edit the file, restart Sacred."
...
- Length-changing edits are fine — script re-computes all subsequent offsets.
...
1. We override CreateWindowExA → set WS_POPUP / fullscreen size.
2. Sacred calls ChangeDisplaySettingsA → we swallow.
3. Sacred then calls `FUN_00811440(hwnd, INFINITE_SENTINEL)` to force-focus.
4. SetForegroundWindow fails under our weird state → busy-wait forever.
5. Black screen + frozen taskbar.

`patch6` makes step 4's function return `false` instantly. Steps 1-3 are now
safe to attempt because step 4 can't hang.
...
- Sacred's UI scales by pixel count — at 4K the HUD will be tiny. ReBorn HD
  also adjusts camera distance for >1366×768; we don't yet (DDraw vtable
  hooks not implemented).
- If the game runs but renders only in the top-left corner at original
  640×480, Sacred is hard-coded to that backbuffer size. We'd need to hook
  `IDirectDraw::SetCooperativeLevel` + `IDirectDrawSurface::Blt` to stretch
  the backbuffer. Future work.
...
1. **Logger** — launch Sacred, play a bit (main menu + load save + few seconds in-game).
   Overlay: "Resource lookup logger" should show non-zero `calls` and `unique`
   counts growing. After exit:
   ```
   python sdk/tools/seen_hashes_resolve.py
   ```
   Verify `sdk/logs/seen_named.csv` exists with rows. Note the rate of
   "with name" vs "(unknown)" — that's our hash-namespace coverage in live use.

2. **Text mod** — grep for some safe string, modify it, restart, see it in-game.
   ```
   ...
   ```
   Find Mick in Bellevue (Sacred starter town); his name now reads "TEST MOD WORKS".
   Restore from `global.res.bak` after.
...
3. **HD force** — copy `sdk.ini.example` -> `sdk.ini`, enable `force_borderless`
   ...
   borderless at requested size on the desktop. If that works, then try
   `swallow_displaymode=1` too — Sacred should now think it's fullscreen.
