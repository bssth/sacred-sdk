...
The SDK edits none of the game's data files. The one file it replaces is
`ijl15.dll`, so Steam's "Verify integrity of game files" restores the original
`ijl15.dll` and the SDK stops loading: run `install.cmd` again afterwards.
Uninstalling is renaming that one DLL back.
...
```lua
-- save as: <game>/custom/lua/mods/kolb.lua
...
V.on_ready(function()                         -- the world is up (new game or save)
  ...
  o:dialog{
    text = "KOLB_OFFER",
    buttons = { { label = S.ACCEPT, on = function() A.run(Vb.add_gold(500)) end },
                { label = S.REJECT } },
  }
end)
...
The mod does its work while the game runs and returns no script records, so it
lives in `custom/lua/mods/`, a folder that mirrors no game file. Do not save a mod
like this at a game script's path such as
`custom/lua/bin/TYPE_NPC_GLADIATOR/FunkCode.lua`: a file there bakes to the same
path under `custom/bin/`, and the game reads that output in place of its own
script, here an empty `FunkCode.bin`.
...
└── custom/                 ← YOURS
    ├── lua/mods/…          ← your runtime mods (they replace no game file)
    ├── lua/bin/…           ← script mods: each replaces the game's bin/ file at its path
    ├── lua/lib/…           ← optional: your copy of a framework module
    ├── bin/, mods/         ← generated by the bake; bin/ is served to the engine
