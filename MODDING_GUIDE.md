...
> **Where things live.** There are two Lua trees and yours wins.
> `sdk/custom/lua/` is the framework that ships with the SDK — the standard
> library and the examples. `custom/lua/` next to `Sacred.exe` is yours, and any
> file you put there beats the file at the same relative path in the SDK's tree,
> module for module and mod for mod. Baked output always goes to your `custom/`
> tree; the SDK's is only ever read, so updating the SDK never touches your work.
>
> Recipes for common tasks: **MODDING_COOKBOOK.md**.
